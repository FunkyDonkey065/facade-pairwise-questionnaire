...
window.SurveyDemographics = (() => {
  ...
  const fields = [
    { name: "age_band", label: "What is your age group?", localOnly: true, options: [
    ...
    { name: "education_level", label: "What is the highest level of education you have completed?", localOnly: true, options: [
  ...
  const forSource = source => fields.filter(field => !field.localOnly || source === "local");
  function metadata(source, response) {
    const complete = Boolean(response);
    return {
      demographics_version: "facade_demographics_v1",
      demographics_status: complete ? "completed_optional_page" : "not_reached",
      age_data_source: source === "prolific" ? "prolific_export_pending" : "questionnaire",
      education_data_source: source === "prolific" ? "prolific_export_pending" : "questionnaire",
      age_band: source === "prolific" ? "external_pending" : response?.age_band || "not_collected",
      education_level: source === "prolific" ? "external_pending" : response?.education_level || "not_collected",
  ...
  class DemographicsPlugin {
    ...
    trial(displayElement, trial) {
      ...
      displayElement.querySelector("form").addEventListener("submit", event => {
        ...
        this.jsPsych.finishTrial({ ...trial.data, ...values, rt: Math.round(performance.now() - start) });
      });
    }
  }
